...
#ifdef MCBL_SS
/*
const int clrwl_opaque0Format = RGBA16F;
const int clrwl_opaque0Target = 8;
*/
#endif

#if defined ADVANCED_MATERIALS && defined REFLECTION_SPECULAR
/*
const int clrwl_opaque1Format = RGBA16F;
const int clrwl_opaque2Format = RGBA16F;
const int clrwl_opaque3Format = RGBA8;

const int clrwl_opaque1Target = 3;
const int clrwl_opaque2Target = 6;
const int clrwl_opaque3Target = 7;
*/
#endif
...
#ifdef ADVANCED_MATERIALS
vec2 dcdx = dFdx(flw_vertexTexCoord);
vec2 dcdy = dFdy(flw_vertexTexCoord);
#endif
...
vec3 FlwToPlayer(vec3 pos) {
	return mat3(flw_viewInverse) * pos + flw_viewInverse[3].xyz - flw_cameraPos;
}
...
void main()
{
	...
	vec2 lmCoord = clamp(((flw_fragLight * 16.0 / 15.0) - 0.03125) * 1.06667, vec2(0.0), vec2(0.9333, 1.0));

	vec4 color = flw_vertexColor;
    vec4 albedo = flw_fragColor;
	...
	float skipParallax = 0.0;
	
	#ifdef PARALLAX
	if (skipParallax < 0.5) {
		newCoord = GetParallaxCoord(flw_vertexTexCoord, parallaxFade, surfaceDepth);
		albedo = texture2DGradARB(flw_diffuseTex, newCoord, dcdx, dcdy) * color;
	}
	#endif
	...
	if (albedo.a > 0.001) {
		...
		vec3 worldPos = FlwToPlayer(viewPos);
...
void main() {
	...
	#ifdef ADVANCED_MATERIALS
	binormal = normalize(clrwl_normal * cross(flw_vertexTangent.xyz, gl_Normal.xyz) * flw_vertexTangent.w);
	...
	dist = length(flw_view * flw_vertexPos);

	vec2 midCoord = flw_vertexMidTexCoord;
	vec2 texMinMidCoord = flw_vertexTexCoord - midCoord;

	vTexCoordAM.pq  = abs(texMinMidCoord) * 2;
	vTexCoordAM.st  = min(flw_vertexTexCoord, midCoord - texMinMidCoord);

	vTexCoord.xy    = sign(texMinMidCoord) * 0.5 + 0.5;
	#endif

	if(flw_vertexColor.a < 0.1) flw_vertexColor.a = 1.0;
    ...
    #ifdef WORLD_CURVATURE
	vec2 positionXZ = flw_vertexPos.xz - flw_cameraPos.xz;
	float curvatureY = WorldCurvature(positionXZ);
	gl_Position = flw_viewProjection * (flw_vertexPos - vec4(0.0, curvatureY, 0.0, 0.0));
	#else
