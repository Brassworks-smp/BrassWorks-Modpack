...
vec3 toClipSpace3(vec3 viewSpacePosition) {
	...
	{
		return texture2DGradARB(tex,fract(coord)*vtexcoordam.pq+vtexcoordam.st,dcdx,dcdy);
	}
#endif
...
void main() {
	...
	vec3 worldpos = mat3(flw_viewInverse) * fragpos + flw_viewInverse[3].xyz - flw_cameraPos;
	
	vec2 adjustedTexCoord = flw_vertexTexCoord;
...
#endif
	if(!ifPOM) adjustedTexCoord = flw_vertexTexCoord;
	...
	flw_sampleColor = texture2D_POMSwitch(tex, adjustedTexCoord.xy, vec4(dcdx,dcdy), ifPOM, textureLOD);
	...
	vec4 fragColor_bkp = flw_fragColor;
    flw_shaderLight();

	vec3 fragColorLightRatio = flw_fragColor.xyz / fragColor_bkp.xyz;
	float ambientOcclusion = max(max(fragColorLightRatio.r, fragColorLightRatio.g), fragColorLightRatio.b);

	vec4 Albedo = flw_fragColor;
	vec2 lmcoord = flw_fragLight;
	...
	gl_FragData[2] = vec4(FlatNormals * 0.5 + 0.5, 1.0 - ambientOcclusion);	

	// CLRWL_DISABLE_AUTO_FRAGCOLOR
	flw_fragColor = Albedo;
}
